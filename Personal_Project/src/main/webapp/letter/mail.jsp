<%@page import="member.MemberBean"%>
<%@page import="member.MemberDAO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>all sort of things</title>
	<link href="../css/home.css" rel="stylesheet" type="text/css">
	<link href="../css/letter.css" rel="stylesheet" type="text/css">
</head>
<body>
<div id="wrap">
<jsp:include page="../home/top.jsp"></jsp:include>
<!-- menu -->
	<nav id="menu">
		<ul>
			<li><a href="../home/main.jsp">Home</a></li>
			<li><a href="../board/list.jsp">Blog</a></li>
			<li><a href="../galary/galaryList.jsp">Galary</a></li>
			<li><a href="../letter/mail.jsp" id="menuOn">Letter</a></li>
		</ul>
	</nav>
	<div id="clear"></div>
<!-- 컴퓨터 -->
	<section id="computer">
		<img src="../images/macintosh (2).png">
	</section>
<!-- mail -->
<%
String id = (String)session.getAttribute("id");
MemberDAO mdao = new MemberDAO();
MemberBean mb = mdao.getMember(id);

%>
	<section id="mail">
		<form action="sendMail.jsp" method="post">
			<table>
				<tr>
					<th>from</th>
					<td><input type="email" name="from" value="<%=mb.getEmail()%>"></td>
				</tr>
				<tr>
					<th>to</th>
					<td><input type="email" name="to"></td>
				</tr>
				<tr>
					<th>title</th>
					<td><input type="text" name="title"></td>
				</tr>
				<tr>
					<th>content</th>
					<td><textarea rows="10" cols="12" name="content" ></textarea></td>
				</tr>
			</table>
			<div id="button">
				<input type="submit" value="send" class="btn">
			</div>
		</form>
	</section>
<jsp:include page="../home/bottom.jsp"></jsp:include>
</div>
</body>
</html>